...
export const Breadboard = ({ data }: { data: ComponentData }) => {
  const pinDots = [];
  // Top half (pinIdx 0-149)
  for (let pinIdx = 0; pinIdx < 150; pinIdx++) {
    const x = -7.25 + Math.floor(pinIdx / 5) * 0.5;
    const z = -2.25 + (pinIdx % 5) * 0.5;
    pinDots.push(<mesh key={`t${pinIdx}`} position={[x, 0.05, z]}><cylinderGeometry args={[0.08, 0.08, 0.05, 8]} /><meshBasicMaterial color="#9ca3af" /></mesh>);
  }
  // Bottom half (pinIdx 150-299)
  for (let pinIdx = 150; pinIdx < 300; pinIdx++) {
    const x = -7.25 + Math.floor((pinIdx-150) / 5) * 0.5;
    const z = 0.25 + ((pinIdx-150) % 5) * 0.5;
    pinDots.push(<mesh key={`b${pinIdx}`} position={[x, 0.05, z]}><cylinderGeometry args={[0.08, 0.08, 0.05, 8]} /><meshBasicMaterial color="#9ca3af" /></mesh>);
  }
  ...
  return (
    <group position={data.position} rotation={data.rotation}>
      ...
      <mesh position={[0, 0.01, -3.5]}><boxGeometry args={[16.5, 0.05, 0.05]} /><meshBasicMaterial color="#ef4444" /></mesh>
      <mesh position={[0, 0.01, -3.0]}><boxGeometry args={[16.5, 0.05, 0.05]} /><meshBasicMaterial color="#3b82f6" /></mesh>
      <mesh position={[0, 0.01, 3.0]}><boxGeometry args={[16.5, 0.05, 0.05]} /><meshBasicMaterial color="#ef4444" /></mesh>
      <mesh position={[0, 0.01, 3.5]}><boxGeometry args={[16.5, 0.05, 0.05]} /><meshBasicMaterial color="#3b82f6" /></mesh>
      {/* Main plane overlay */}
