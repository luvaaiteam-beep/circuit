...
const replaceStr2 = `                    <button onClick={handleRotate} className="w-full py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 rounded text-xs font-medium flex items-center justify-center gap-2">
      ...
      {/* DUMMY END `; // To allow replacing the rest out

// We need to match the end of the block properly.
// Let's just find the index of targetStr2 and then index of `<div className="fixed inset-x-0 bottom-0 z-[200] bg-zinc-950 border-t border-zinc-800 rounded-t-2xl flex flex-col p-4 shadow-[0_-5px_30px_rgba(0,0,0,0.5)]">` (the More Actions menu)
